...
router.get("/user/des_hist/:id", async (req, res) => {
    ...
    const query = {
        text: "select titulo, idmetadato, to_char(fecha, 'DD/MM/YYYY') as fecha, to_char(hora, 'HH24:MI:SS') as hora from descargas inner join metadato on id_metadato=idmetadato where id_usuario=$1;",
        values: [id]
    }
    try {
        var hist = []
